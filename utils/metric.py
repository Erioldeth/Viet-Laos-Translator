...
def bleu_single(model, valid_dataset, debug=False):
	"""
	Perform single sentence translation, then calculate bleu score.
	Update when batch beam search is online
	"""
...
def bleu_batch(model, valid_dataset, batch_size, debug=False):
	...
	predictions = model.translate_batch_sentence([s.src for s in valid_dataset], output_tokens=True,
