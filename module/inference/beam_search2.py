...
def generate_subsequent_mask(sz, device):
	return (torch
	        .triu(torch.ones(sz, sz, dtype=torch.int, device=device))
	        .transpose(0, 1)
	        .unsqueeze(0))
...
class BeamSearch2(BeamSearch):
	...
	def beam_search(self, src, src_lang=None, trg_lang=None, src_tokens=None, n_best=1, debug=False):
		...
		results = [self._convert_to_sent(trg[t, j.item(), :], eos_token_id) for t, j in
		           enumerate(torch.argmax(log_scores, dim=-1))]
		return results
